...
class FetchRewardDataFromKoiosServiceImplTest {
  ...
  @Mock
  RewardCheckpointRepository rewardCheckpointRepository;
  @Mock
  PoolHistoryCheckpointRepository poolHistoryCheckpointRepository;
  @Mock
  ...
  @BeforeEach
  void setUp() {
    fetchRewardDataService = new FetchRewardDataFromKoiosServiceImpl(rewardCheckpointRepository, poolHistoryCheckpointRepository, poolUpdateRepository);
    ReflectionTestUtils.setField(fetchRewardDataService, "apiCheckRewardUrl", apiCheckRewardUrl);
    ReflectionTestUtils.setField(fetchRewardDataService, "apiCheckPoolHistoryUrl", apiCheckPoolHistoryUrl);
  ...
  void fetchRewardStakeKeySetTest(){
    final Set<String> stakeKeySet = Collections.singleton("stake1u8n707tj5wr83na6dw3lcwzlluu5p0yq88mh4gg6z02htxql0rp92");
  ...
  void checkPoolHistoryForPoolTest(){
    final Set<String> poolIds = Collections.singleton("pool1u8n707tj5wr83na6dw3lcwzlluu5p0yq88mh4gg6z02htxql0rp92");
    when(poolHistoryCheckpointRepository.checkRewardByPoolViewAndEpoch(poolIds))
        .thenReturn(1);

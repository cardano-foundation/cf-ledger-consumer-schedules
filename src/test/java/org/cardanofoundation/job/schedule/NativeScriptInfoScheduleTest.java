...
public class NativeScriptInfoScheduleTest {
  ...
  @Captor
  ArgumentCaptor<List<NativeScriptInfo>> argumentCaptorNativeScriptInfo;
  ...
  void syncNativeScriptInfo_shouldInitDataWhenSyncDataFirstTime() {
    ...
    Script script = Script.builder()
        ...
        .hash(scriptHash)
        .json("{\"type\":\"all\",\"scripts\":[{\"type\":\"sig\",\"keyHash\":\"26bacc7b88e2b40701387c521cd0c50d5c0cfa4c6c6d7f0901395757\"},{\"type\":\"before\",\"slot\":23069343}]}")
        .build();
    ...
    ScriptNumberTokenProjection scriptNumberTokenProjection = Mockito.mock(ScriptNumberTokenProjection.class);
    ...
    ScriptNumberHolderProjection scriptNumberHolderProjection = Mockito.mock(ScriptNumberHolderProjection.class);
    ...
    verify(nativeScriptInfoRepository, Mockito.times(1)).saveAll(argumentCaptorNativeScriptInfo.capture());
  ...
  void syncSmartContractInfo_shouldUpdateDataWhenSyncDataNotFirstTime() {
    ...
    Script script = Script.builder()
        ...
        .type(ScriptType.TIMELOCK)
        .hash(scriptHash)
        .json("{\"type\":\"sig\",\"keyHash\":\"89d555c7a028bc560ea44e52a81c866088566f3a99c9989a5a183940\"}")
        .build();

    when(scriptRepository.findAllByHashIn(List.of(scriptHash)))
        .thenReturn(List.of(script));
    when(nativeScriptInfoRepository.findAllByScriptHashIn(Set.of(scriptHash)))
        .thenReturn(Collections.emptyList());

    when(multiAssetRepository.findPolicyByTxIn(3L, 10L, List.of(ScriptType.TIMELOCK, ScriptType.MULTISIG)))

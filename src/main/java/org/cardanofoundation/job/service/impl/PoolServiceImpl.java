  ...
  public PoolStatus getCurrentPoolStatus() {
    ...
    var mPoolRetireCertificate =
        poolRetireRepository.getLastPoolRetireTilEpoch(currentEpoch).stream()
            .collect(
                Collectors.toMap(
                    PoolUpdateTxProjection::getPoolId, PoolUpdateTxProjection::getTxId));
    ...
    poolCertAndTxId.forEach(
        poolUpdateTxProjection -> {
          if (!mPoolRetireCertificate.containsKey(poolUpdateTxProjection.getPoolId())
              || poolUpdateTxProjection.getTxId()
                  > mPoolRetireCertificate.get(poolUpdateTxProjection.getPoolId())) {

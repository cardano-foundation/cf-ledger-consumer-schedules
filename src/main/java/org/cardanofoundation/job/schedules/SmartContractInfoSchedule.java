...
import org.cardanofoundation.explorer.common.entity.ledgersync.Script_;
...
public class SmartContractInfoSchedule {
  ...
  void saveSmartContractInfo(List<Script> scriptList) {
    ...
    List<String> scriptHashes =
        scriptList.stream().map(Script::getHash).collect(Collectors.toList());

    Map<String, Long> sContractTxCntList =
        redeemerRepository.getSContractTxCountByHashIn(scriptHashes).stream()
            .collect(
                Collectors.toMap(
                    SContractTxCntProjection::getScriptHash, SContractTxCntProjection::getTxCount));

    Map<String, List<SContractPurposeProjection>> sContractPurposeList =
        redeemerRepository.getScriptPurposeTypeByScriptHashIn(scriptHashes).stream()
            .collect(Collectors.groupingBy(SContractPurposeProjection::getScriptHash));

    // get existing smart contract info
    Map<String, SmartContractInfo> smartContractInfoMap =
        smartContractInfoRepository.findAllByScriptHashIn(scriptHashes).stream()
    ...
    smartContractInfoNeedSave.addAll(
        scriptList.stream()
            .filter(script -> !smartContractInfoMap.containsKey(script.getHash()))
            .map(
                script ->
    ...
    smartContractInfoNeedSave.forEach(
        smartContractInfo -> {
          ...
          Set<ScriptPurposeType> sContractPurposeProjectionList =
              sContractPurposeList
                  .getOrDefault(smartContractInfo.getScriptHash(), new ArrayList<>())
                  .stream()

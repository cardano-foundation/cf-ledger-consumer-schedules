...
public class S3Configuration implements BeanFactoryAware {
  ...
  public S3Configuration(S3Properties s3Properties) {this.s3Properties = s3Properties;}
  ...
  public void onPostConstruct() {
    ...
    for (S3Config s3Bean : s3Beans) {
      ...
      AmazonS3 amazonS3 = switch (s3Bean.getProfile()) {
        case S3_PROFILE ->
            buildAmazonS3(s3Bean.getAccessKey(), s3Bean.getSecretKey(), s3Bean.getRegion());
        case MINIO_PROFILE ->
            buildAmazonS3Clone(s3Bean.getEndpoint(), s3Bean.getAccessKey(), s3Bean.getSecretKey(),
                               s3Bean.getRegion(), s3Bean.getBucket());
  ...
  private AmazonS3 buildAmazonS3Clone(String serviceEndpoint, String accessKey, String secretKey,

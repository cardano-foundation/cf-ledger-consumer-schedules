...
public class JOOQLatestTokenBalanceRepository {
  ...
  public void init() {
    ...
    // WITH latest_token_balance_tmp as
    //    ( SELECT DISTINCT ON (address, unit) address, unit, slot, quantity, block_time
    //      FROM address_balance
    //      WHERE unit <> 'lovelace'
    //      ORDER BY address, unit, slot desc )
    // INSERT INTO latest_token_balance
    // SELECT ltb.address, a.stake_address, substring(cast(ltb.unit as varchar), 1, 56) AS "policy",
    // ltb.slot, ltb.unit, ltb.quantity, ltb.block_time
    // FROM latest_token_balance_tmp ltb
    // INNER JOIN address AS a ON ltb.address = a.address;
    ...
    log.info("Init Latest Token Balance {} ms", System.currentTimeMillis() - startTime);
  }
